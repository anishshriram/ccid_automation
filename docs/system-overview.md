...
Wrapping all of this: **[cli-lifecycle-and-monitoring.md](cli-lifecycle-and-monitoring.md)** (the process entry point, signal handling, systemd watchdog, outbound monitoring) and **[tools.md](tools.md)** (commissioning/calibration/offline-analysis CLIs, all built on the same production code paths). **[test-suite-guide.md](test-suite-guide.md)** maps all 349 tests back to the behaviors described across every doc above.
