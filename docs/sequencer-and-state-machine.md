...
2. **The forced-diagnostic checkpoint** — `forced_diagnostic_deadline = k3_closed_s + _FORCED_DIAGNOSTIC_DELAY_S` (100ms — comfortably inside the 300ms backstop). At this point, `_issue_forced_diagnostic_trigger` runs exactly once (`forced_diagnostic_checked` is a single-fire latch). This exists because of `SCOPE_TRIGGER_DEBUG_LOG.md` Entry 11: real-hardware evidence of TER=0 for the entire 306.6ms K3-closed window confirmed a genuine no-trigger condition, and the team wanted diagnostic visibility into what the analog front end actually looks like when the real trigger doesn't fire.
