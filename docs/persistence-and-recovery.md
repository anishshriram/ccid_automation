...
**Source files:** `ccid/recorder.py` (571 lines), `ccid/config.py` (502 lines)
...
**`write_controller_exception_diagnostics`** is the newest of the three, added after a real incident where a `controller:unexpected:ValueError` halt's actual traceback was lost — non-persistent journald, Pi power-cycled after the halt, nothing durable survived. Called from `Sequencer._capture_controller_exception_diagnostics`, itself called from `_run_cycle`'s last-resort `except Exception as exc:` handler (sequencer doc §8.3), before the exception gets collapsed down to just `type(exc).__name__` for the halt reason. Writes one file, `diagnostics/<cycle_index>/controller_exception.json`: exception type, exception message, the full formatted traceback (`traceback.format_exception`), the last known `CycleState`, the complete per-cycle transition list, and both the capture and cycle-start monotonic timestamps. Same three properties as the other two methods above (no `RunState`, writes only under `diagnostics/`, best-effort) — plus one more, unique to this one: the *caller* (`_capture_controller_exception_diagnostics`) wraps the whole call in its own `try/except`, because this is invoked from the sequencer's absolute last line of defense, and a second failure here must never mask the first exception it's trying to document.
...
### 8.1 Structure
`AppConfig` is a frozen dataclass tree: `GpioConfig`, `VisionConfig`, `CameraHardwareConfig`, `TimingConfig`, `ModesConfig`, `PathsConfig`, `MonitoringConfig`, plus `analysis: AnalysisConfig` (imported directly from `ccid.analysis` — see that doc §2, not redefined here). `_validate_and_build` walks the raw YAML dict section by section; every scalar goes through one of `_require_int`/`_require_float`/`_require_str`/`_require_int_or_str`/`_require_mapping`/`_require_non_empty_path`, each raising a specific `ConfigValidationError` message rather than letting a `TypeError`/`KeyError` leak out with a confusing traceback. Note `_require_int`/`_require_float` both explicitly reject `bool` (`isinstance(value, bool)` is checked *before* `isinstance(value, int)`, since `bool` is a subclass of `int` in Python and `True`/`False` would otherwise silently pass as `1`/`0`).

`CameraHardwareConfig.device_index` is the one field using `_require_int_or_str` — a non-negative int (a raw `/dev/videoN` index) or a non-empty string (a stable device path, e.g. a udev-provided symlink) are both accepted, since `cv2.VideoCapture()` takes either natively. Added after a real incident: a hardcoded numeric index isn't stable across USB reconnects (the camera re-enumerated from `video0` to `video1` mid-campaign, leaving `device_index: 0` pointing at nothing) — `config.yaml` now points at `/dev/ccid_camera`, a symlink `deploy/99-c270-camera.rules` creates by matching the camera's USB serial rather than its enumeration order.
...
| `camera.device_index` accepts a stable string path, rejects an empty string | `test_accepts_a_stable_device_path_string_for_camera_device_index`, `test_rejects_empty_string_camera_device_index` |
...
- **Any new field added to `AppConfig` needs a decision about `_canonical_for_hash`**: does a change to this field mean a resumed run is now running under meaningfully different behavior (→ include it in the hash) or not (→ deliberately exclude it, and say why, like `monitoring`'s secret-exclusion above)? `asdict()` picks up every new dataclass field automatically, so nothing has to be opted in by hand — every `timing.*` addition (`equipment_refresh_interval_cycles`, `equipment_refresh_after_consecutive_camera_unavailable`) is included in the hash the same way every other `timing.*` field already was.
...
- If you add a new diagnostic-write method (alongside `write_timeout_diagnostics`/`write_forced_diagnostic_capture`/`write_controller_exception_diagnostics`), keep it outside the crash-safe contract on purpose — don't give it a `RunState` parameter or let it touch `runstate.json`, or you've quietly expanded what "committed" means.
- If a new field genuinely needs to accept more than one type (like `device_index`'s int-or-path), add a dedicated `_require_*` helper (`_require_int_or_str` is the current example) rather than loosening an existing one's semantics for every other caller.
