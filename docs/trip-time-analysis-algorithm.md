...
This is worth having in one place since it's scattered across code comments, `scope-trigger-debug-log.md`, and this session's conversation.
...
It also deliberately never touches any Pi-side monotonic timestamp — a real defect this session found and fixed (`scope-trigger-debug-log.md` Entry 13): the Pi's `time.monotonic()` clock and the scope's own internal waveform timebase are not synchronized by anything in this system, so an earlier attempt to say "the burst occurred 86ms before K3 close" by comparing a Pi timestamp to a scope-timebase offset was unsupported. Every number `analyze_forced_diagnostic_waveform` produces comes from `load_waveform`'s scope-preamble-derived time base alone (`x_increment`/`x_origin`), which is self-consistent on its own.
