...
This exists to gather a best-effort snapshot after the scope has already failed to complete an acquisition — i.e., precisely the moment the scope is most likely to be in a bad state. Every design choice here traces back to a specific real-hardware incident recorded in `SCOPE_TRIGGER_DEBUG_LOG.md`:
