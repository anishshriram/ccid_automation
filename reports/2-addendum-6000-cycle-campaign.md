...
Project A.M.P.E.R.E., specified in full in the accompanying System Design and Validation report, was built to exercise a CCID's trip-time behavior across far more cycles than a manual test bench can practically produce. This addendum reports what happened when it was: a combined 6,000-cycle endurance campaign, run across three separate attempts on three different days rather than one unbroken sitting, producing 5,883 PASS, 116 FAIL, and 1 NO_TRIP verdict out of 5,999 cycles with a recorded trip time. It covers the campaign's structure and provenance, the exact preflight and execution procedure followed, the incidents that occurred during the run, the resulting trip-time distribution and its behavior over time (overall and broken out per run), a direct evidenced investigation into endpoint sensitivity for cycles near the pass/fail boundary, a verification of the instrumentation's timebase and channel-coupling configuration against real campaign data, a reconstruction of when the auto-retry logic engaged, an independent offline cross-check of the recorded verdicts using two structurally distinct analysis methods, and a consolidated list of open follow-up items. Consistent with a deliberate design decision in the System Design report (Requirement OPS/MEAS discipline, §9.1 of that report), no campaign-level pass/fail acceptance criterion is proposed or implied anywhere in this document — the numbers are reported, not graded.
...
The preflight sequence executed before each of the three real campaigns follows `docs/deployment-and-operator-runbook.md` §3 directly. This section reproduces the specific checks and commands actually run, rather than summarizing them as a single paragraph.

### 4.1 Physical and Repository Preflight

With EVSE mains and all three 12 V contactor coil supplies off: qualified visual inspection of the setup; protective-earth connections intact; K1/K2/K3 wiring and driver-channel labeling verified against the as-built assignment (K1 = L1/GPIO17, K2 = L2/GPIO27, K3 = leakage/GPIO22); probe attenuation switch confirmed at 10x on the approved measurement node; camera mechanically secure and aimed at the EVSE LED; no loose conductors or visible damage; emergency disconnect accessible; test area restricted during energized operation.

On the Pi itself:

```bash
cd ~/ccid_automation && source venv/bin/activate
git status --short              # expect: clean, only known temporary untracked files
git log -3 --oneline --decorate # expect: local matches origin/development
pgrep -af "python.*ccid.main" || echo "No active CCID process"  # must be empty
df -h "$HOME"                    # expect: > 2 GB free (min_free_disk_gb)
```

### 4.2 Software Test Gate and Analysis-Version Confirmation

```bash
mkdir -p ~/ccid_test_tmp
TMPDIR="$HOME/ccid_test_tmp" env -u CCID_SCOPE_RESOURCE python -m unittest discover -s tests -p 'test_*.py'
# expect: OK, 364 tests, exactly the 2 documented intentional skips
grep -n "algorithm_version" config.yaml   # expect: v3
```

A failing test suite or a non-v3 algorithm version is disqualifying for a real-hardware campaign, per the runbook's own rule — none of the three real runs behind this addendum proceeded past a failing gate at this step.

### 4.3 Camera, Contactor, and Oscilloscope Preflight

Camera:
```bash
v4l2-ctl --list-devices                                                  # confirms the Logitech C270 enumerates
...
v4l2-ctl --device=/dev/video0 --get-fmt-video                            # expect: 640x480 YUYV
```
A warmed frame (≥3 s of active reads) is captured and inspected: full LED visible, near-centered inside the configured ROI, exposure not washed out on green.

Contactors (mains off, 12 V coil supplies on):
```bash
python -m tools.gpio_selftest exercise --contactor K1 --real --i-understand-this-energizes-hardware --pulses 1 --hold-s 0.5 --cooldown-s 1
python -m tools.gpio_selftest exercise --contactor K2 --real --i-understand-this-energizes-hardware --pulses 1 --hold-s 0.5 --cooldown-s 1
python -m tools.gpio_selftest exercise --contactor K3 --real --i-understand-this-energizes-hardware --pulses 1 --hold-s 0.2 --cooldown-s 1
```
Each contactor confirmed to pull in once and release cleanly, no buzzing or sticking, final safe-off completed.

Oscilloscope:
```bash
lsusb | grep -i "0957:1798"                          # expect: Agilent Technologies, Inc. MSO-X 2014A
export CCID_SCOPE_RESOURCE="USB0::2391::6040::MY58100795::0::INSTR"
python -m tools.scope_bench identify --real          # expect: Model MSO-X 2014A, Serial MY58100795
python -m tools.scope_bench configure --real         # expect: CH1, +20V positive edge, 50ms/div, RAW/BYTE, no ScopeConfigurationError
```
The SCPI error queue is polled directly and confirmed clean (`:SYSTem:ERRor?` returning `+0,"No error"`) before proceeding.

### 4.4 Network and Monitoring Preflight (Autonomous Runs)

For an unattended campaign: `ip -brief address show usb0` / `ip route get 8.8.8.8` confirm the dedicated Verizon USB-tethered hotspot — not a phone hotspot — is the active route; `CCID_CRONITOR_URL` is confirmed present in `/etc/default/ccid-automation`, and a normal heartbeat, a failure heartbeat, and recovery are each verified before the run starts, since the Cronitor monitor is known to auto-recreate itself after deletion and its lifecycle is not assumed correct from a prior campaign.
...
```bash
python -m ccid.main --config config.yaml start --target-cycles <N> --run-id <RUN_ID>
```

A **supervised** run executes this directly over an SSH session that must remain open for the run's duration. An **autonomous** run wraps the identical command in a transient `systemd-run` unit tied to a freshly generated, never-reused run ID, so it continues if the SSH session disconnects:
...
Neither pattern differs in what runs — only in what happens if the operator disconnects. A third, persistent, always-`enable`d systemd service also exists in this project's deployment configuration (`docs/deployment-and-operator-runbook.md` §3.13) but has never been used to produce any of the three real campaigns in this addendum, or any other real campaign to date (`docs/build-and-commissioning-issue-log.md` §6); every real run has used one of the two patterns above. This addendum's own evidence is consistent with that split rather than silent about it: the 483-cycle run's three reconstructed process pauses (§9.1) — including two confirmed process restarts spread across roughly four calendar days — are the signature of a supervised session being started, stopped, and resumed by an operator across separate visits, not of an autonomous service left running unattended. Which specific pattern produced the 200-cycle and 5,317-cycle runs is not independently recorded in the retained data; both completed to their target with no internal evidence of a mid-run process restart, which is consistent with either pattern and does not by itself distinguish them.

Once started, the same rule applies regardless of pattern: nothing about scope or camera controls is touched manually during the run, and a halt is never resumed automatically without a human first reading why it happened.

### 4.6 Post-Run Verification

Applied to each of the three runs once fully de-energized, before treating its results as trustworthy:

```bash
python -m ccid.main --config config.yaml status --run-id <RUN_ID>
```
checked against: `last_completed_cycle` matches the target or the expected halt point; `halt_reason` is null for a normal completion; PASS/FAIL counts are plausible.

```bash
cat runs/<RUN_ID>/cycles.csv
```
reviewed for internal consistency on every accepted PASS: trip time strictly positive; computed `t_end >= t0`; `analysis_version = v3`; every waveform sanity check true; camera state `CHARGING` at gate grant; no unexplained `degraded_flags`.

```bash
find runs/<RUN_ID> -maxdepth 3 -type f -printf '%p %s bytes\n' | sort
```
confirmed the expected artifact set present for every completed cycle (`cycles/<n>.json`, `waveforms/<n>.npz`, `images/<n>_scope.png`, `images/<n>_green.jpg`) plus the run-root files (`config.yaml`, `cycles.csv`, `runstate.json`). No cycle's data, including a failed or otherwise invalid one, was discarded during this process for any of the three runs.
...
was fit two ways: once across the full combined timeline, and once restricted to the 5,317-cycle run alone, since that is the only one of the three attempts both long enough and internally continuous enough (§9.1) for a same-sitting drift trend to mean anything on its own.
...
The slowest-PASS/fastest-FAIL gap is 11.0 µs — smaller than several of the definitional offsets discussed in §8 and in the System Design report's uncertainty budget (§5.5 of that report). §5.4 below reports a direct, evidenced investigation into exactly these two cycles, rather than leaving the margin as an abstract number.

### 5.4 Near-Boundary Endpoint Sensitivity

The 11.0 µs gap in §5.3 motivated a direct inspection of both waveforms' raw samples around their reported `t_end`, rather than treating the margin as an abstract number.

**What the raw samples show.** By the time each waveform reaches its reported `t_end`, the signal has already decayed to the scope's own ADC quantization floor — the samples bounce between 0 V, 2.01 V, and 4.02 V, exactly 0, 1, and 2 counts of this scope's 2.01005 V per-count resolution (System Design report §5.5). There is no visibly decaying current left at that point; it is electrical noise at the resolution floor of the instrument.

**Why this specific mechanism can move an endpoint.** The production algorithm's final refinement step (System Design report §5.2) walks the collapse point forward, past the initial envelope-threshold crossing, looking for genuine sub-threshold residual conduction down to `residual_floor` — a deliberate design choice, since real current sometimes tapers off gradually rather than stopping instantly. `residual_floor` is derived from each waveform's own estimated noise level (`5 × noise_sigma`, System Design report §5.2, §5.4.4) and is not guaranteed to sit meaningfully above the ADC's own quantization step. For both of these two specific cycles, `residual_floor` computes to approximately 4.0 V — almost exactly 2 ADC counts. At that level, an ordinary noise sample landing on 4.02 V purely by chance (which both recordings show happening more than once) is indistinguishable, to the refinement search, from genuine residual current — meaning the exact instant chosen as `t_end` for two of the most boundary-defining cycles in this entire campaign is plausibly landing on noise, not signal.

**Scope of this finding.** This is a direct, evidenced observation from hand-inspecting the two specific cycles that define the tightest real margin in the dataset — not a full audit of every near-boundary cycle. To bound how many cycles are structurally exposed to the same mechanism, cycles were counted by distance from the 24.97 ms pass limit:

| Distance from the pass limit | Cycle count (of 5,999) |
|---|---|
| Within 50 µs | 23 |
| Within 100 µs | 48 |
| Within 500 µs | 239 |
...
**What this does, and does not, change about §5.1–§5.3's conclusions.** Even in an implausible worst case — every one of the 239 within-500-µs cycles being mislabeled by this mechanism — the campaign's headline pass rate would move by a few tenths of a percent, not change the reported story; §5.1's confidence intervals already comfortably absorb a shift of that size. The distribution shape, the drift analysis, and the single unambiguous NO_TRIP (§5.6) are unaffected, since none of those conclusions rest on margins anywhere near this scale. What is **not** fully established is the individual verdict on any specific cycle within roughly half a millisecond of the 24.97 ms line: for such a cycle, the honest statement is that it is *probably* correctly classified, but a real, evidenced mechanism exists that could be nudging a `t_end` on either side of the line, and it has not been fixed or re-validated against this campaign's data.

**Recommended follow-up**, not undertaken as part of this addendum (also carried into §13): require `residual_floor` to sit a firmer, quantified margin above the ADC's own quantization step rather than allowing it to land almost on top of it, and/or require the forward residual-tail search to confirm more than one isolated sample before extending `t_end` — the same sustained-run confirmation principle V3 already applies to onset detection (System Design report §5.4.3), extended to the collapse side. Per this project's own versioning discipline (System Design report, Requirement MEAS-03), any such change ships as a new `AnalysisVersion`, validated and replayed against this campaign's archived waveforms, never as an in-place edit to V3.
...
The three runs' pass rates and trip-time distributions are consistent with one another — every pair of per-run confidence intervals overlaps substantially, and the mean/median trip times differ by well under a millisecond across all three. Nothing in this breakdown suggests the 200- and 483-cycle runs (collected under a slightly different operational configuration — no equipment refresh, a raw-index camera path rather than the udev-resolved one, §3.2) behaved differently from the 5,317-cycle run in any way that would call into question combining them into one dataset for the aggregate statistics in §5.2–5.4.

### 5.6 NO_TRIP Detail

The single NO_TRIP verdict occurred at cycle 3,115 of the 5,317-cycle run (global cycle index 3,798), recorded 2026-08-19T19:32:45 UTC. Its analysis record shows no envelope collapse anywhere within the full captured window — the fault current was still conducting at the end of the record, not merely slow to clear — and nothing about that waveform's own signal-quality figures (reference amplitude 166.83 V, noise sigma 0.977 V) stands out as anomalous relative to the rest of the campaign; there is no indication this was a capture or equipment problem rather than a genuine non-clearing event. All three independent cross-check methods (§8) agree with this call — none found a collapse in this waveform either. The raw waveform itself, plotted directly (Appendix B), shows sustained AC conduction continuing across multiple full mains cycles past $t_0$ with no sign of an approaching collapse — visually confirming the "still conducting, not merely slow" reading the algorithm's own notes record. This same waveform is also the basis for the channel-coupling check in §6.2, since it is the longest sustained real-conduction event anywhere in the campaign.

## 6. Measurement Chain Configuration and Verification

Two configuration questions about the instrumentation chain — the oscilloscope's timebase setting and the channel's coupling mode — were raised and investigated directly against this campaign's own data during the preparation of this addendum. Both are reported here because they were resolved with evidence, not assumption.

### 6.1 Timebase: Sample Interval and the 20 ms/div Question

**What `ms/div` actually controls.** The oscilloscope's timebase (`timebase_scale_s_per_div`, 50 ms/div per the locked `ScopeSettings`, System Design report Appendix C) sets the total time window a single acquisition spans — for this scope's 10-division display, 50 ms/div gives a 500 ms captured window. Separately, `waveform_points` is configured to `MAXimum`, meaning the scope always transfers as many points as its memory depth allows for whatever window is showing. The actual per-sample time spacing is therefore

$$dt = \frac{\text{captured window}}{\text{points captured}},$$

not a fixed instrument speed. This is confirmed directly against a real captured preamble from this campaign's data: at 50 ms/div, the preamble reports exactly 1,000,000 points and `x_increment = 5\times10^{-7}\ \text{s}` (0.5 µs, a 2 MHz effective sample rate) — matching $500\ \text{ms} / 1{,}000{,}000 = 0.5\ \mu\text{s}$ exactly. This confirms, with a real number rather than the order-of-magnitude estimate in the System Design report's uncertainty budget (§5.5 of that report), the sample interval this campaign's data was actually captured at.

**Why a shorter division setting would not improve accuracy.** This oscilloscope is memory-depth-limited, not acquisition-speed-limited, at both 50 ms/div and 20 ms/div — a shorter window still fills close to the same ~1,000,000-point memory, spread across less time. Switching to 20 ms/div (a ~200 ms window) would therefore *shrink* `dt` to roughly 0.2 µs rather than widen it. That sounds like an improvement, but the verdict decision (System Design report §5.3) only uses `dt/2` as a quantization margin at the pass/no-trip boundaries: 0.25 µs at the current 0.5 µs sample interval, against a 24.97 ms threshold — about one part in 100,000 of the threshold itself. Shrinking that to 0.1 µs is a difference of a tenth of a microsecond, three orders of magnitude smaller than the algorithm's own known endpoint-refinement bias (a few hundred microseconds, System Design report §5.5; §5.4 of this addendum quantifies a real instance of it) or the systematic offsets between independent cross-validation methods (§8.2, ±1.9–2.4 ms). Sample-clock resolution is not, and was never, the limiting factor on this system's measurement precision.

**Why a shorter division setting would cost real safety margin.** Halving the captured window from ~500 ms to ~200 ms is a real cost, not a neutral trade. With the centered timebase this system uses, that shrinks the post-trigger portion from roughly 250 ms to roughly 100 ms — and the no-trip limit itself is 100 ms, measured from $t_0$ (the detected true onset), not from the hardware trigger event, which can fire up to half a mains cycle after $t_0$ (System Design report §5.1). A ~100 ms post-trigger record, with some of that margin already consumed by trigger-to-onset lag, risks falling under the window the `SANITY_RECORD_SPANS_NO_TRIP_LIMIT` check exists specifically to guarantee — one of only two sanity checks in the entire pipeline permitted to halt the rig outright (System Design report §5.3, Requirement MEAS-04). Shrinking the pre-trigger portion the same way would also weaken the pre-trigger-leakage (K3-stuck-closed) check's own quiet baseline.

**Conclusion.** 50 ms/div was retained rather than switched to 20 ms/div: the resolution gain is real but three to four orders of magnitude smaller than this system's actual, evidenced sources of endpoint uncertainty, while the window reduction directly erodes the safety margin the current setting was deliberately chosen to provide, for exactly the marginal and no-trip cases where measurement confidence matters most. `ms/div` is also a horizontal/time setting exclusively — it has no bearing on voltage/amplitude precision, which is governed by V/div and the scope's 8-bit ADC (System Design report §5.5) instead; a general concern about "measurement accuracy" does not live in the timebase setting at all.

### 6.2 Channel Coupling: AC vs. DC

Channel 1 is configured with AC coupling (System Design report Appendix C), which inserts a high-pass filter between the probe and the digitizer — the textbook-correct choice for viewing a small signal riding on a large steady bias, but not, in principle, the textbook-correct choice for faithfully capturing a one-shot transient's true amplitude for its full duration, since an AC-coupled input lets a sustained level droop toward zero over time at a rate set by the coupling network's own time constant. This matters most for exactly the case where current is expected to keep flowing longest: the campaign's single NO_TRIP cycle (§5.6), whose conduction persists for nearly 250 ms.

That cycle's raw waveform was checked directly for amplitude droop, rather than assuming the theoretical concern does or does not apply here: RMS amplitude was measured in successive 10 ms windows across the full conducting period, from onset to the end of the record. No droop was found — amplitude cycles between roughly 108 V and 125 V repeatedly throughout, an oscillation consistent with the ordinary beat pattern of a 10 ms RMS window against a 60 Hz signal that doesn't divide evenly into the mains period, not decay. The pattern at the start of the window (t ≈ 0–50 ms) and at the end (t ≈ 200–240 ms) is essentially identical; a real AC-coupling droop over nearly 15 mains cycles of sustained conduction would show a clear downward trend, and none is present.

This is evidence from one cycle, the longest-duration real conduction event in the dataset, not a full characterization of this scope's AC-coupling corner frequency. Within that scope, AC coupling does not appear to have measurably distorted this campaign's results. DC coupling remains the more textbook-correct choice for a one-shot transient capture in general, and is flagged as a future configuration change to consider (§13) — not because this data shows a problem, but because the current evidence is one waveform, not a calibrated characterization of the coupling network's actual time constant.
...
**Offset-correction procedure, stated explicitly**: the correction subtracts each method's own median offset from the production algorithm's value (Table above — the median, not the mean, to reduce sensitivity to the tail cases in §8.3), computed once across all 5,999 waveforms with a recorded trip time and applied uniformly; no boundary cycles were excluded when computing the offset, and the same offset is applied regardless of a cycle's distance from either configured limit.
...
### 8.3 A Genuine Limitation Found, Not Just Disclaimed
...
### 9.1 Mid-Run Process Pauses (`5800_v3_try2`)
...
### 9.3 Correlations and Secondary Observations
...
### 9.4 Sanity-Check and Data-Integrity Summary
...
What this campaign supports saying, directly from its own data: the rig completed 6,000 real fault-injection cycles across three attempts with no safety-relevant incident — no pre-fault leakage detection ever fired, no waveform sanity check ever failed (§9.4), no cycle's own record shows any sign of an interlock behaving unexpectedly. The measurement and analysis pipeline behaved with complete internal consistency across the entire campaign, and the one auto-retry event and the process pauses within the 483-cycle run were each recovered from cleanly, without losing, duplicating, or misattributing a single cycle's data. The independent cross-check in §8 found no evidence that the production algorithm's real-time-safe design is missing something a heavier offline method would catch — and the direct hand-inspection in §5.4 found one specific, quantified mechanism that could affect verdicts for cycles very close to the pass/fail boundary, reported honestly rather than left undiscovered.

What this campaign does not, and cannot, support saying is whether a 98.05% pass rate, or the specific pattern of FAILs observed, constitutes an acceptable result for the device under test — that judgment was deliberately excluded from this system's own design from the start (System Design report §8) and remains excluded here. It is also worth being precise about what this campaign does not validate: nothing in this addendum, including §8's cross-check, independently confirms the oscilloscope's own calibration, trigger timing accuracy, or sample-rate accuracy — every method compared in this document works from the same captured samples, so a systematic problem upstream of those samples would be invisible to all of them at once (the same gap the System Design report's uncertainty budget names explicitly, §5.5 of that report). Several limitations already known before this campaign began also remain genuinely open after it: the measurement endpoint definition is still this project's own provisional reading of the governing standard, contactor state is still only ever known as commanded rather than confirmed, and protective-earth continuity is still only inferred indirectly.

### 10.2 Lessons Learned

The clearest thread connecting this campaign back to the commissioning story in the System Design report is that a system judged trustworthy in advance can still surface things nobody anticipated once it is actually run at scale, and the right response, demonstrated repeatedly during commissioning and repeated again here, is to look for those things deliberately rather than stop looking once the obvious checks pass. Retaining full raw timestamps on every cycle, not just a summary log, is what made it possible to reconstruct the three process pauses in the 483-cycle run after the fact (§9.1) — nothing in the system's own explicit logging captured that those pauses happened, or why. Running a genuinely independent second analysis method against the same data, even with no specific reason to expect it would disagree, is what turned up both a real limitation in a candidate alternative method (§8.3's fast-trip window bias) and an unexplained structural pattern (the banded cross-validation scatter) that a simple "does it agree with the production number" check would have missed entirely. And directly inspecting the raw samples behind the tightest real margin in the dataset, rather than treating an 11 µs gap as a number to report and move past, is what surfaced §5.4's residual-floor/quantization finding — a lesson in its own right about which margins are worth opening the waveform for.

The two open structural questions this campaign leaves behind — the trip-time distribution's three distinct peaks (§5.2) and the stepped structure in the cross-validation comparison (§8.3) — are, on their own, a reminder that a campaign with a completely clean sanity-check record (§9.4) is not the same thing as a campaign with nothing left to investigate.

## 11. Limitations

Carried forward, unresolved by this campaign: contactor physical-state readback; independent protective-earth continuity monitoring; confirmation of the measurement endpoint definition against the published UL 2231-2 text (Requirement MEAS-05, System Design report §9.1); replay of the earlier archived 25-cycle dataset under the current V3 algorithm version; the persistent, always-enabled deployment pattern, still never exercised in producing a real campaign (this addendum's own process-pause evidence in §9.1 is, if anything, more consistent with supervised sessions than with any such pattern having run continuously); the external monitoring service's full pause/resume lifecycle, not independently reconfirmed by anything in this document; full watchdog, reboot, and true power-loss recovery commissioning, as distinct from the unplanned process interruptions this campaign happened to produce and recover from; and the calibration-grade uncertainty terms (oscilloscope timebase accuracy, probe tolerance) that are not available in this repository (System Design report §5.5).

**New in this addendum:** the collapse-side residual-floor endpoint-refinement mechanism identified in §5.4 — evidenced directly against the two cycles that define this campaign's tightest real margin, quantified as structurally relevant to roughly 4% of the dataset (239 of 5,999 cycles within 500 µs of the boundary), and not yet fixed or re-validated.

Partially addressed by this campaign: the auto-retry mechanism was exercised for real, exactly once, and resolved cleanly (§7), real evidence toward — though not exhaustive proof of — its correctness under an actual repeat condition. The equipment-refresh mechanism's real-world activation remains unconfirmed either way (§9.2). The first-cycle zero-trip-time behavior noted as unresolved in the System Design report did not recur anywhere in this campaign — all three runs' own opening cycles produced ordinary, unremarkable trip-time readings — though its underlying cause was never identified, so its absence here should be read as "did not happen to occur in this data," not as "fixed."

New to this campaign, not anticipated going in: the tri-modal shape of the trip-time distribution (§5.2); the banded, stepped structure in the alternative-algorithm comparison (§8.3), possibly related to the first item but not established as such; the cause of the three mid-run process pauses in the 483-cycle run (§9.1), for which no root cause survives in the retained data; whether the moderate correlation between trip time and each waveform's reference amplitude and noise level (§9.3) reflects a genuine property of the device under test or is partly an artifact of the production algorithm's own amplitude-derived thresholds; and the residual-floor endpoint-sensitivity mechanism itself (§5.4). None of these five is resolved in this document.

## 12. Conclusions

The System Design and Validation report closed by stating that whether the system performed as intended at scale was a question it could not answer in the abstract, and left that question to this addendum. Across three attempts totaling 6,000 real fault-injection cycles, it did: every cycle completed safely, the crash-safe persistence design recovered cleanly from every process interruption the campaign happened to encounter, the one auto-retry event resolved on its next attempt, and the analysis pipeline's own sanity checks passed without a single exception anywhere in the data. An independent, non-authoritative second analysis method, built from scratch specifically to look for disagreement, found none that could not be explained by the methods' own differing definitions of a measurement endpoint — and a direct, hands-on investigation of the tightest real margin in the dataset found one genuine, quantified mechanism worth fixing, reported plainly rather than smoothed over.

The trip-time results themselves are reported in full in §5 and are not restated or graded here. This addendum has deliberately stopped short of judging whether they represent an acceptable outcome for the device under test, consistent with the system's own design principle that a campaign-level acceptance decision belongs to a human, made offline, not to anything this document or the software that produced its data infers on its own. What this addendum has established is narrower and, in its own way, more foundational: that the system performed reliably enough, and transparently enough, for its own results to be trusted as an accurate account of what actually happened during those 6,000 cycles, with one specific, bounded exception now on record — which is the precondition any acceptance judgment built on top of this data would depend on.

## 13. Recommendations and Open Actions

Consolidated from the scattered open-items lists in §11 above and the System Design report's own §8 and §9.1, in one place:
...
- **Tighten the collapse-side residual-floor endpoint refinement (§5.4)** — require a firmer, quantified margin above the ADC's own quantization step, and/or a sustained-run confirmation on the collapse side matching V3's existing onset-confirmation principle (System Design report §5.4.3). Ship as a new, explicitly validated `AnalysisVersion`, replayed against this campaign's archived waveforms, per the project's own versioning discipline — never as an in-place edit.
- Investigate the trip-time distribution's tri-modal structure (§5.2) and the banded cross-validation scatter (§8.3) — flagged here as connected-but-unconfirmed open questions.
...
**Instrumentation configuration to reconsider for future campaigns (no evidence of current harm):**
- Consider DC coupling on channel 1 (§6.2) — the more textbook-correct choice for a one-shot transient capture; current AC-coupling evidence (one waveform, the campaign's single NO_TRIP) shows no droop, but is not a calibrated characterization of the coupling network's actual time constant.
- 50 ms/div was evaluated against 20 ms/div and retained (§6.1) — no action needed, recorded here so the decision and its reasoning aren't re-litigated from scratch next time it comes up.
...
- Continue accumulating real-world evidence for auto-retry and equipment-refresh correctness — each has now been exercised for real exactly once (§7, §9.2).
...
**Table A.10 — Near-boundary cycle counts (distance from the 24.97 ms pass limit, of 5,999)**
...
*Verdict timeline strip, marking every FAIL and the single NO_TRIP across the combined 6,000-cycle sequence (§5.1, §5.6).*
...
*The single NO_TRIP waveform — `5317_v3_real` cycle 3,115 (global 3,798). Sustained AC conduction continues across multiple full mains cycles past $t_0$ with no envelope collapse anywhere in the record (§5.6); also the basis for the AC-coupling droop check in §6.2.*
